...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        mask = 1.0 * (np.random.uniform(size=x.shape) > p)
        out = x * mask
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    flat_w = np.reshape(w, (F, -1))
    for i in range(H_new):
        for j in range(W_new):
            for n in range(N):
                flat_image_crop = padded_x[n,:,stride*i:(stride*i + HH),stride*j:(stride*j + WW)].flatten()
                for f in range(F):
                    out[n,f,i,j] = np.dot(flat_w[f], flat_image_crop) + b[f]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def conv_backward_naive(dout, cache):
    ...
    for i in range(H_new):
        for j in range(W_new):
            for n in range(N):
                for c in range(C):
                    padded_dx[n,c,stride*i:(stride*i + HH),stride*j:(stride*j + WW)] += np.sum(w[:,c,:,:] * np.expand_dims(np.expand_dims(dout[n,:,i,j], 1), 2), axis=0)
                    for f in range(F):
                        dw[f,c,:,:] += padded_x[n,c,stride*i:(stride*i + HH),stride*j:(stride*j + WW)] * dout[n,f,i,j]
                        # dout.shape=(N, F, H', W')
...
def max_pool_forward_naive(x, pool_param):
    ...
    H_new = int((H - pH) / stride + 1)
    W_new = int((W - pW) / stride + 1)
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
        for c in range(C):
            for i in range(H_new):
                for j in range(W_new):
                    a = x[n, c, (i * stride):(i * stride + pH), (j * stride):(j * stride + pW)]
                    max_indeces = np.unravel_index(a.argmax(), a.shape)
                    dx[n, c, i * stride + max_indeces[0], j * stride + max_indeces[1]] = dout[n, c, i, j]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    ###########################################################################
    N, C, H, W  = x.shape
    reshaped_x = np.swapaxes(x, 1 ,2) # N, H, C, W
    reshaped_x = np.swapaxes(reshaped_x, 2 ,3) # N, H, W, C
    reshaped_x = np.reshape(reshaped_x, (N * H * W, C))
    out, cache = batchnorm_forward(reshaped_x, gamma, beta, bn_param)
    out = np.reshape(out, (N, H, W, C))
    out = np.swapaxes(out, 2 ,3) # N, H, C, W
    out = np.swapaxes(out, 1 ,2) # N, C, H, W
...
def spatial_batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    N, C, H, W  = dout.shape
    dout_reshaped = np.swapaxes(dout, 1 ,2) # N, H, C, W
    dout_reshaped = np.swapaxes(dout_reshaped, 2 ,3) # N, H, W, C
    dout_reshaped = np.reshape(dout_reshaped, (N * H * W, C))
    dx_reshaped, dgamma, dbeta = batchnorm_backward(dout_reshaped, cache)
    dx_reshaped = np.reshape(dx_reshaped, (N, H, W, C))
    dx_reshaped = np.swapaxes(dx_reshaped, 2 ,3) # N, H, C, W
    dx_reshaped = np.swapaxes(dx_reshaped, 1 ,2) # N, C, H, W
